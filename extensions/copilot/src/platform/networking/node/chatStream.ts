...
export function sendEngineMessagesTelemetry(telemetryService: ITelemetryService, messages: CAPIChatMessage[], telemetryData: TelemetryData, isOutput: boolean, logService?: ILogService) {
	...
	logService?.trace(`[engine.messages] (${dir}) properties: ${JSON.stringify(multiplexProperties(telemetryDataWithPrompt.properties))}, measurements: ${JSON.stringify(telemetryDataWithPrompt.measurements)}`);
